...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="ProjectPython - Discord Bot's launcher")
    parser.add_argument("--start", "-s",
                        help="Starts ProjectPython",
                        action="store_true")
    parser.add_argument("--auto-restart",
                        help="Autorestarts ProjectPython in case of issues",
                        action="store_true")
    parser.add_argument("--update-projectpython",
                        help="Updates ProjectPython (git)",
                        action="store_true")
...
def update_projectpython():
    try:
        ...
        return
    if code == 0:
        print("\nProjectPython has been updated")
    else:
        print("\nProjectPython could not update properly. If this is caused by edits "
              "you have made to the code you can try the repair option from "
              "the Maintenance submenu")


def reset_projectpython(reqs=False, data=False, cogs=False, git_reset=False):
    if reqs:
        try:
    ...
    if git_reset:
        ...
        if code == 0:
            print("ProjectPython has been restored to the last local commit.")
        else:
...
def download_ffmpeg(bitness):
    clear_screen()
    repo = "https://github.com/Jerlag01/ProjectPython/tree/master/"
    verified = []
...
def update_menu():
    ...
    while True:
        ...
        print("Update:\n")
        print("ProjectPython:")
        print("1. Update ProjectPython + requirements (recommended)")
        print("2. Update ProjectPython")
...
def maintenance_menu():
    ...
    while True:
        print(INTRO)
        print("Maintenance:\n")
        print("1. Repair ProjectPython (discards code changes, keeps data intact)")
        print("2. Wipe 'data' folder (all settings, modules' data...)")
        ...
        if choice == "1":
            print("Any code modification you have made will be lost. Data/"
                  "non-default modules will be left intact. Are you sure?")
            if user_pick_yes_no():
                reset_projectpython(git_reset=True)
                wait()
        elif choice == "2":
            ...
            if user_pick_yes_no():
                reset_projectpython(data=True)
                wait()
        elif choice == "3":
            reset_projectpython(reqs=True)
            wait()
        elif choice == "4":
            print("Are you sure? This will wipe ALL your ProjectPython's installation "
                  "data.\nYou'll lose all your settings, cogs and any "
                  "modification you have made.\nThere is no going back.")
            if user_pick_yes_no():
                reset_projectpython(reqs=True, data=True, cogs=True, git_reset=True)
                wait()
        elif choice == "0":
            break
        clear_screen()
...
def run_projectpython(autorestart):
    interpreter = sys.executable
    ...
    if verify_requirements() is None:
        print("You don't have the requirements to start ProjectPython. "
              "Install them from the launcher.")
        if not INTERACTIVE_MODE:
            exit(1)

    cmd = (interpreter, "projectpython.py")
    ...
    while True:
        try:
            ...
            if code == 0:
                ...
                print("Restarting ProjectPython...")
                continue
            else:
                if not autorestart:
                    break

    print("ProjectPython has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of ProjectPython without going
    ...
    start_projectpython = "{} --start".format(call)
    start_projectpython_autorestart = "{} --start --auto-restart".format(call)
    modified = False
    ...
    start_projectpython             = ccd + start_projectpython             + pause
    start_projectpython_autorestart = ccd + start_projectpython_autorestart + pause

    files = {
        "start_projectpython"             + ext : start_projectpython,
        "start_projectpython_autorestart" + ext : start_projectpython_autorestart
    }
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE ProjectPython Discord Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        if not is_git_installation:
            print("WARNING: It doesn't look like ProjectPython has been "
        ...
        print("1. Run ProjectPython /w autorestart in case of issues")
        print("2. Run ProjectPython")
        print("3. Update")
        ...
        choice = user_choice()
        if choice == "1":
            run_projectpython(autorestart=True)
        elif choice == "2":
            run_projectpython(autorestart=False)
        elif choice == "3":
            update_menu()
        elif choice == "4":
            requirements_menu()
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("ProjectPython needs Python 3.5 or superior. Install the required "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("ProjectPython cannot work without the pip module. Please make sure to "
              "install Python without unchecking any option during the setup")
        wait()
        exit(1)
    if args.repair:
        reset_projectpython(git_reset=True)
    if args.update_projectpython:
        update_projectpython()
    if args.update_reqs:
        install_reqs(audio=True)
    ...
    if INTERACTIVE_MODE:
        ...
        print("Starting ProjectPython...")
        run_projectpython(autorestart=args.auto_restart)
