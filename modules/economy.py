...
class Economy:
    ...
    async def register(self, ctx):
        """Registers an account at the ProjectPythons bank"""
        ...
        try:
            ...
            await self.bot.say("{} You already have an account at the"
                               " ProjectPythons bank.".format(author.mention))
        ...
        if not user:
            ...
            try:
                ...
                await self.bot.say("{} You don't have an account at the"
                                   " ProjectPythons bank. Type `{}bank register`"
    ...
    async def _set(self, ctx, user: discord.Member, credits: SetParser):
        ...
        Examples:
            bank set @Jerlag 26 - Sets 26 credits
            bank set @Jerlag +2 - Adds 2 credits
            bank set @Jerlag -6 - Removes 6 credits"""
        author = ctx.message.author
        try:
...
def setup(bot):
    ...
    check_folders()
    check_files()
    logger = logging.getLogger("projectpython.economy")
    if logger.level == 0:
