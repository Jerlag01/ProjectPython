...
log = logging.getLogger("projectpython.owner")
...
class OwnerUnloadWithoutReloadError(Exception):
    pass

class Owner:
    """All owner-only commands that relate ti debug bot operations."""

    def __init__(self, bot):
        self.bot = bot
        self.setowner_lock = False
        self.disabled_commands = dataIO.load_json("data/projectpython/disabled_commands.json")
        self.global_ignores = dataIO.load_json("data/projectpython/global_ignores.json")
    ...
    async def _show_modules(self):
        """Shows loaded/unloaded moduless"""
        # This function assumes that all moduless are in the moduless folder,
    ...
    async def _set(self, ctx):
        """Changes ProjectPython's core settings"""
    ...
    async def owner(self, ctx):
        ...
        if self.bot.settings.no_prompt is True:
            await self.bot.say("Console interaction is disabled. Start ProjectPython "
    ...
    async def prefix(self, ctx, *prefixes):
        """Sets ProjectPython's global prefixes
    ...
    async def serverprefix(self, ctx, *prefixes):
        """Sets ProjectPython's prefixes for this server
    ...
    async def game(self, ctx, *, game=None):
        """Sets ProjectPython's playing status
    ...
    async def stream(self, ctx, streamer=None, *, stream_title=None):
        """Sets ProjectPython's streaming status
    ...
    async def _token(self, token):
        """Sets ProjectPython's login token"""
        if len(token) < 50:
    ...
    async def _blacklist_add(self, user: GlobalUser):
        """Adds user to ProjectPython's global blacklist"""
    ...
    async def _blacklist_remove(self, user: GlobalUser):
        """Removes user from ProjectPython's global blacklist"""
    ...
    async def whitelist(self, ctx):
        ...
        be able to use ProjectPython"""
        if ctx.invoked_subcommand is None:
            await self.bot.send_cmd_help(ctx)

    @whitelist.command(name="add")
    async def _whitelist_add(self, user: GlobalUser):
        """Adds user to ProjectPython's global whitelist"""
    ...
    async def _whitelist_remove(self, user: GlobalUser):
        """Removes user from ProjectPython's global whitelist"""
    ...
    async def restart(self, silently : bool=False):
        """Attempts to restart ProjectPython

        Makes ProjectPython quit with exit code 26
    ...
    @checks.is_owner()
    async def join(self):
        """Shows ProjectPython's invite URL"""
    ...
    @commands.command()
    async def info(self):
        """Shows info about ProjectPython"""
        author_repo = "https://github.com/Jerlag01"
        projectpython_repo = author_repo + "/ProjectPython"
        server_url = "https://projectpython.flanderscraft.be"
        ...
        embed.add_field(name="About ProjectPython", value=about, inline=False)
    ...
    async def version(self):
        """Shows ProjectPython's current version"""
    ...
    def _load_module(self, modulename):
        ...
        try:
            ...
            raise ModuleLoadError(*e.args)
        except:
            raise

    def _unload_module(self, modulename, reloading=False):
        if not reloading and modulename == "modules.owner":
    ...
    def _does_modulefile_exist(self, module):
        if "modules." not in module:
            module = "modules." + module
        if module not in self._list_modules():
            return False
        return True
    ...
    def _get_version(self):
        if not os.path.isdir(".git"):
            msg = "This instance of ProjectPython hasn't been installed with git."
    ...
    def save_global_ignores(self):
        dataIO.save_json("data/projectpython/global_ignores.json", self.global_ignores)

    def save_disabled_commands(self):
        dataIO.save_json("data/projectpython/disabled_commands.json", self.disabled_commands)
...
def check_files():
    if not os.path.isfile("data/projectpython/disabled_commands.json"):
        print("Creating empty disabled_commands.json...")
        dataIO.save_json("data/projectpython/disabled_commands.json", [])

    if not os.path.isfile("data/projectpython/global_ignores.json"):
